...
function mostrar_post_type($query)
{
	...
	if (!is_admin() && $query->is_main_query()) {
		// que sea home page
		if (is_home()) {
			// add recetas al query
			$query->set('post_type', ['post', 'recetas']);
		}
	}
}
...
function recetas_comer()
{
	...
	foreach ($posts as $post) {
		// agregar informacion al post, imagenes y otras cosas
		setup_postdata($post);
		$listado[] = array(
			'objeto' => $post,
			'id' => $post->ID,
...
function filtrar_platillos($busqueda)
{
	...
	$args = array(
		...
		'tax_query' => array(
			array(
				...
				'field' => 'slug',
				// ? find by comida-italiana and internacional at the same time
				// 'terms' => array('comida-italiana', 'internacional'),
				'terms' => $busqueda,
			)
		),
	);
	...
	while ($comida->have_posts()) : $comida->the_post();
		...
		echo get_the_post_thumbnail(get_the_ID(), 'platilloBuscado');
		echo '</a>';
